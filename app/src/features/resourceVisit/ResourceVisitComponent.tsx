...
import { useGetResourceVisitsQuery } from "./resourceVisitApi";
...
const HostVisitComponent: React.FC = () => {
  ...
  const maxDate = new Date(Date.UTC(2024, 7, 31, 23, 59, 59));
  ...
  const { data, error, isLoading } = useGetResourceVisitsQuery({
    fetchOption,
    range: timePeriod,
  });
