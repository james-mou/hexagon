...
export const resourceVisitApi = createApi({
  reducerPath: "resourceVisitApi",
  baseQuery: baseQueryWithParserErrorHandling,
  endpoints: (builder) => ({
    getResourceVisits: builder.query<
      TabResponse,
    ...
    >({
      query: ({ fetchOption, range }) => {
        ...
        return {
          url: `visits-by-resource?${queryParams.toString()}`,
          method: "GET",
        };
      },
    }),
...
export const { useGetResourceVisitsQuery } = resourceVisitApi;
